...
// --- Helper Functions ---
const getPublicationStatusBadge = (isApproved) => {
  if (isApproved) {
    return (
...
const PaperList = ({ papers, onRemind, onSendBack, onViewFile, onBulkApprove }) => {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      // Only select papers that are NOT approved yet
      const selectablePapers = sortedPapers
        .filter(p => !p.CompletedPublication) 
        .map(p => p.id);
  ...
  const handleApproveClick = () => {
    // Check if any selected paper is missing required files
    const selectedPapersData = papers.filter(p => selectedIds.includes(p.id));
    const invalidPapers = selectedPapersData.filter(p => !p.FinalPaperURL || !p.CopyrightURL);

    if (invalidPapers.length > 0) {
      const invalidIds = invalidPapers.map(p => p.id).join(", ");
      alert(`Cannot approve the following papers because Final Paper or Copyright is missing:\n${invalidIds}`);
      return;
    }
  ...
  const selectableCount = sortedPapers.filter(p => !p.CompletedPublication).length;
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-[#e5e7eb] bg-gray-50">
            ...
            {/* Status Column */}
            <th className="text-left py-3 px-4 text-sm font-medium text-[#6b7280]">
              <button onClick={() => requestSort('CompletedPublication')} className={headerBtnClass}>
                Status {getSortIndicator('CompletedPublication')}
              </button>
            </th>
        ...
        <tbody>
          {sortedPapers.length > 0 ? (
            sortedPapers.map((paper) => (
              <tr 
                key={paper.id} 
                className={`border-b border-[#e5e7eb] transition-colors ${paper.CompletedPublication ? 'bg-gray-50' : 'hover:bg-[#f3f4f6]/50'}`}
              >
                <td className="py-3 px-4">
                  <input
                    ...
                    disabled={paper.CompletedPublication}
                ...
                <td className="py-3 px-4">
                  {paper.FinalPaperURL ? (
                    <button onClick={() => onViewFile(paper.FinalPaperURL)} className="text-sm text-[#059669] hover:underline text-left">
                      View Final Paper
                    </button>
                  ) : (
                ...
                <td className="py-3 px-4">
                  {paper.CopyrightURL ? (
                    <button onClick={() => onViewFile(paper.CopyrightURL)} className="text-sm text-[#059669] hover:underline text-left">
                      View Copyright
                    </button>
                  ) : (
                    <span className="text-sm text-red-500 font-medium">Missing</span>
                ...
                {/* Status Badge */}
                <td className="py-3 px-4">
                   {getPublicationStatusBadge(paper.CompletedPublication)}
                </td>
                ...
                <td className="py-3 px-4">
                  <div className="flex justify-center gap-2">
                    <button
                      ...
                      disabled={paper.isProcessing || paper.CompletedPublication}
            ...
            <tr>
              <td colSpan="9" className="text-center text-gray-500 py-4">No papers found.</td>
            </tr>
          )}
        </tbody>
      </table>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 space-y-4">
        ...
        <p className="text-sm text-gray-600">
          This action will mark the paper as <b>Pending</b>. Please provide a reason for the author.
        </p>
...
export default function PublicationChairPortal() {
    ...
    const fetchPapers = async () => {
      try {
        ...
        const formattedPapers = (data.paper || []).map((p) => ({
          ...
          CompletedPublication: p.CompletedPublication,
          isProcessing: false
        }));
  ...
  const handleSendBackConfirm = async (paper, message) => {
    try {
      ...
      // Update local state: CompletedPublication -> false
      setPapers(prev => prev.map(p =>
        p.id === paper.id ? { ...p, CompletedPublication: false, isProcessing: false } : p
      ));

      alert("Paper sent back to author. Status updated to Pending.");
    ...
    try {
      ...
      setPapers(prevPapers =>
        prevPapers.map(paper =>
          paperIds.includes(paper.id) ? { ...paper, CompletedPublication: true } : paper
        )
      );
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb]">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        <button onClick={() => navigate("/conference/manage/publicationchair")} className="mb-4 text-[#059669] hover:text-[#047857] font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-[#e5e7eb] rounded-lg p-5 shadow-sm">
            <h3 className="text-sm text-[#6b7280]">Approved</h3>
            <p className="text-3xl font-bold text-[#059669]">{papers.filter(p => p.CompletedPublication).length}</p>
          </div>
          <div className="bg-white border border-[#e5e7eb] rounded-lg p-5 shadow-sm">
            <h3 className="text-sm text-[#6b7280]">Pending Approval</h3>
            <p className="text-3xl font-bold text-[#f59e0b]">{papers.filter(p => !p.CompletedPublication).length}</p>
          </div>
        </div>
