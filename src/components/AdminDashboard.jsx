...
export default function AdminDashboard() {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      if (res.ok) {
        ...
        fetchGlobalStats(); // Refresh dashboard stats automatically
      }
    } catch (error) { 
  ...
  return (
    <div className="min-h-screen bg-[#ffffff]">
      {/* HEADER - Updated with Conferences & Journal Navigation */}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-end mb-8">
            ...
            <div className="text-sm font-medium text-[#059669] bg-[#059669]/10 px-3 py-1 rounded-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="bg-[#f9fafb] border border-[#e5e7eb] rounded-lg p-5 flex flex-col justify-between shadow-sm">
                ...
                <div className="flex items-end justify-between mt-2">
                    ...
                    <span className="text-xs font-semibold text-[#059669] bg-green-100 px-2 py-1 rounded">Active</span>
                </div>
              </div>
              ...
              <div className="bg-[#f9fafb] border border-[#e5e7eb] rounded-lg p-5 flex flex-col justify-between shadow-sm">
                ...
                <div className="flex items-end justify-between mt-2">
                    ...
                    <span className="text-xs font-semibold text-gray-600 bg-gray-200 px-2 py-1 rounded">Across all events</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                              ...
                              <tbody className="divide-y divide-[#e5e7eb]">
                                  {stats.recentActivity.conferences.map(conf => (
                                      <tr key={conf.id} className="hover:bg-[#f9fafb] transition-colors">
                                          ...
                                          <td className="p-3">
                                              <button 
                                                onClick={() => handleViewConference(conf.id)} 
                                                className="px-3 py-1.5 text-xs font-medium text-[#059669] border border-[#059669]/30 rounded bg-[#059669]/5 hover:bg-[#059669]/10 transition-colors"
                                              >
                                                View Details
                                              </button>
                                          </td>
                  ...
                  <div className="bg-white border border-[#e5e7eb] rounded-lg shadow-sm">
                      <div className="p-4 border-b border-[#e5e7eb]">
                          <h3 className="text-lg font-semibold text-[#1f2937]">Latest User Registrations</h3>
                      </div>
                      <ul className="divide-y divide-[#e5e7eb]">
                          {stats.recentActivity.users.map(u => (
                              <li key={u.id} className="p-4 flex items-center justify-between hover:bg-[#f9fafb] transition-colors">
                                  <div className="flex items-center gap-3">
                                      <div className="h-10 w-10 rounded-full bg-[#059669]/10 flex items-center justify-center text-[#059669] font-bold">
                                          {u.firstname.charAt(0)}{u.lastname.charAt(0)}
                                      </div>
                                      <div>
                                          <p className="text-sm font-medium text-[#1f2937]">{u.firstname} {u.lastname}</p>
                                          <p className="text-xs text-[#6b7280]">{u.email}</p>
                                      </div>
                                  </div>
                                  <div className="text-right">
                                      <p className="text-xs text-[#6b7280] font-medium uppercase">Roles</p>
                                      <p className="text-xs text-[#1f2937]">{u.role.slice(0, 2).join(', ')}{u.role.length > 2 ? '...' : ''}</p>
                                  </div>
                              </li>
                          ))}
                      </ul>
              ...
              <div className="space-y-8">
                  ...
                  <div className="bg-[#f9fafb] border border-[#e5e7eb] rounded-lg p-5 shadow-sm">
                      <h3 className="text-lg font-semibold text-[#1f2937] mb-4">Platform Demographics</h3>
                      
                      <div className="space-y-4">
                          <div>
                              <div className="flex justify-between text-sm mb-1">
                                  <span className="font-medium text-[#1f2937]">Authors</span>
                                  <span className="text-[#6b7280]">{stats.users.authors} users</span>
                              </div>
                              <div className="w-full bg-gray-200 rounded-full h-2">
                                  <div className="bg-[#059669] h-2 rounded-full" style={{ width: `${Math.min((stats.users.authors / stats.users.total) * 100, 100)}%` }}></div>
                              </div>
                          </div>

                          <div>
                              <div className="flex justify-between text-sm mb-1">
                                  <span className="font-medium text-[#1f2937]">Reviewers</span>
                                  <span className="text-[#6b7280]">{stats.users.reviewers} users</span>
                              </div>
                              <div className="w-full bg-gray-200 rounded-full h-2">
                                  <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${Math.min((stats.users.reviewers / stats.users.total) * 100, 100)}%` }}></div>
                              </div>
                          </div>
                      </div>
                  </div>
      ...
      {/* DETAILED CONFERENCE MODAL WITH APPROVE BUTTON */}
      {showConfModal && (
            ...
            <div className="p-6 overflow-y-auto">
              {isLoadingConf ? (
                 ...
                 <div className="space-y-8">
                    ...
                    {/* Header Info & Status/Approve actions */}
                    <div className="flex justify-between items-start">
                       ...
                       <div className="flex flex-col items-end gap-3">
                           ...
                           {/* Inline Approve Button for Pending Conferences */}
                           {selectedConf.status === 'Pending Approval' && (
                             <button 
                               onClick={() => handleApprove(selectedConf.id)} 
