...
// --- NEW: Revision History Table Component ---
const RevisionHistoryList = ({ revisions }) => {
    ...
    const filteredAndSortedRevisions = useMemo(() => {
        if (!revisions) return [];
        ...
        const filtered = revisions.filter(revision => {
            ...
            return (
                revision.version?.toString().includes(lowerSearch) ||
                revision.id?.toString().includes(lowerSearch)
            );
        });

        // 2. Sort
        const getSortValue = (revision, key) => {
            switch (key) {
                case 'version': return revision.version || revision.id || '';
                case 'submittedAt': return revision.submittedAt ? new Date(revision.submittedAt).getTime() : 0;
                default: return '';
            }
        };
    ...
    }, [revisions, sortBy, sortOrder, searchTerm]);

    if (!revisions || revisions.length === 0) {
        return <p className="text-center text-gray-500 py-4">No revision history available for this paper.</p>;
    }

    return (
        <div className="bg-white rounded-lg shadow overflow-hidden border border-[#e5e7eb]">
            <div className="overflow-x-auto">
                <div className="p-4 border-b border-[#e5e7eb]">
                    <input
                        type="text"
                        placeholder="Search revisions (Version, ID...)"
                        value={searchTerm}
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-[#e5e7eb] bg-gray-50">
                            <th onClick={() => handleSort("version")} className="text-left py-3 px-4 text-sm font-medium text-[#6b7280] cursor-pointer hover:text-[#1f2937] whitespace-nowrap">
                                Version / ID {sortBy === "version" && (sortOrder === "asc" ? "↑" : "↓")}
                            </th>
                            <th onClick={() => handleSort("submittedAt")} className="text-left py-3 px-4 text-sm font-medium text-[#6b7280] cursor-pointer hover:text-[#1f2937] whitespace-nowrap">
                    ...
                    <tbody>
                        {filteredAndSortedRevisions.length > 0 ? (
                            filteredAndSortedRevisions.map((revision, index) => (
                                <tr key={revision.id || index} className="border-b border-[#e5e7eb] hover:bg-[#f3f4f6]/50 transition-colors">
                                    ...
                                    <td className="py-3 px-4">
                                        <div className="flex justify-start">
                                            <button 
                                                onClick={() => window.open(revision.URL, '_blank')} 
...
export default function JournalReviewPaper() {
    ...
    // journal papers do not have a "conference" property
    ...
    const handleSubmitReview = async (e) => {
        ...
        if (window.confirm(`Are you sure you want to submit this review as "${recommendation}"? This action cannot be undone.`)) {
            try {
                ...
                navigate("/journal/ManageReviews"); // Navigate only on success
                // --- END FIX ---
    ...
    return (
        <div className="min-h-screen bg-[#ffffff]">
            ...
            <main className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {/* --- NEW: Revision History Section --- */}
                <div className="mt-8">
                    <h3 className="text-xl font-bold text-[#1f2937] mb-4">Revision History</h3>
                    <RevisionHistoryList revisions={paper.Revisions || []} />
                </div>
